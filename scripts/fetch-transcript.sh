...
# 注意: YouTube自動字幕はWhisperエラーで文字化けが多い
# 実測テキスト: 「時は令和ビジネス戦国自体ビジネスの常識をバッサバッサ叩ききるマーケティング侍の非常識なビジネス国ちゃおマーケティング侍のりです」
BOILERPLATE_PATTERNS = [
    # イントロ+自己紹介の一括除去（「時は令和」〜「のりです」まで）
    r'時は令和ビジネス.{5,80}マーケティング侍の.{0,3}(り|龍|竜)です',
    # 自己紹介だけ残った場合のフォールバック
    r'(ちゃ?お?)?マーケティング侍の.{0,5}(り|龍|竜)です',
    # チャンネル説明定型句
    r'このチャンネルではですね.{0,50}(チャンネル|公開|シェア).{0,10}',
    r'このチャンネルでは.{0,10}(実践的な|今すぐ使える).{0,30}(シェアしたい|公開)',
    # アウトロ定型句
    r'チャンネル登録よろしくお願いします',
